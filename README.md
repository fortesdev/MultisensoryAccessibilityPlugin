...
10. The "interpretation" of the relative positions between the sound emitter and the player can also be adapted to the user's perspective. For instance, in first person, the direction the player see matches the FirstPlayerController's pawn. But that's not the case in third person, where your character mesh can be looking directly at the camera (at the actual player), and the pawn's back is actually the camera's front. You can decide what perspective will be taken into consideration with the component's blueprint boolean *isObservedInFirstPerson*. By default, it is false, so the player's pawn view is considered.
